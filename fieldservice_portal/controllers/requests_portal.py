...
class RequestsPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "fsm_location_count" in counters:
            values["fsm_location_count"] = (
                request.env["fsm.location"]
                .sudo()
                .search_count(self._requests_location_domain())
            )
        return values
    ...
    def portal_requests(self, **kw):
        locations = (
            request.env["fsm.location"]
            .sudo()
            .search(self._requests_location_domain(), order="name asc")
        )
        if not locations:
        ...
        if len(locations) == 1:
            return request.redirect(f"/my/requests/{locations[0].id}")
        return request.render(
    ...
    @http.route(
        "/my/requests/<int:location_id>", type="http", auth="user", website=True
    )
    def portal_requests_location(self, location_id, **kw):
        ...
        return request.render(
            "fieldservice_portal.portal_requests_location",
            {
                "location": location,
                "orders": orders,
                "page_name": "requests",
    ...
    @http.route(
        "/my/requests/<int:location_id>/new", type="http", auth="user", website=True
    )
    def portal_requests_new(self, location_id, **kw):
        ...
        if not location:
            return request.redirect("/my/requests")
        return request.render(
            "fieldservice_portal.portal_requests_new",
            {
                "location": location,
                "page_name": "requests",
            },
