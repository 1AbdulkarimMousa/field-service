...
class VisitPortal(CustomerPortal):
    ...
    def portal_request_visit(self, **kw):
        ...
        states = (
            ...
            .sudo()
            .search([("country_id", "=", company.country_id.id)], order="name")
        )
        regions = (
            request.env["res.region"]
            .sudo()
            .search([("state_id", "in", states.ids)], order="name")
        )
        districts = (
            request.env["res.district"]
            .sudo()
            .search([("region_id", "in", regions.ids)], order="name")
        )
        values = {
            "partner": partner,
            "page_name": "visit",
    ...
    def submit_visit_request(self, **kw):  # noqa: C901
        ...
        try:
            ...
            if kw.get("state_id"):
                ...
                state = (
                    ...
                    .sudo()
                    .search(
                        [("id", "=", state_id), ("country_id", "=", country.id)],
                        limit=1,
                    )
                )
                if not state:
            ...
            if (
                ...
                or (
                    manual_location
                    and (not state or not (region or manual_region) or not city)
                )
            ):
            ...
            if kw.get("district_id"):
                ...
                district = (
                    request.env["res.district"]
                    .sudo()
                    .search([("id", "=", district_id), *district_domain()], limit=1)
                )
            ...
            with request.env.cr.savepoint():
                ...
                location_partner = (
                    request.env["res.partner"].sudo().create(location_partner_vals)
                )
                location = (
