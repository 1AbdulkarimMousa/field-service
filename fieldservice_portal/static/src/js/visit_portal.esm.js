    ...
    function loadGoogleMaps() {
        ...
        _gmapsReady = rpc("/website/google_maps_api_key", {}).then(function (resp) {
            ...
            if (!key) {
                ...
                message.textContent = document
                    .getElementById("visitMapKeyMissing")
                    .textContent.trim();
            ...
            return new Promise(function (resolve, reject) {
                ...
                var mapLanguage =
                    document.getElementById("vbf").dataset.mapLanguage || "en";
                s.src =
                    "https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places,geometry&language=" +
                    encodeURIComponent(mapLanguage) +
                    "&callback=_gmapsCallback&key=" +
    ...
    function initVisitBooking() {
        ...
        try {
            ...
            function goToStep(step) {
                document.querySelectorAll(".vbf-section").forEach(function (s) {
                ...
                if (step === 1) {
                    ...
                    document.getElementById("vstepLine1").className =
                        "vbf-step-line done";
                    ...
                    document.getElementById("vstepLine1").className =
                        "vbf-step-line done";
                    document.getElementById("vstepLine2").className =
                        "vbf-step-line done";
                    loadRoutes();
                }
            ...
            function applyManualStepper() {
                ...
                document.querySelector("#vstep2 .vbf-step-label").textContent =
                    translatedText("visitManualStepAddress");
                document.querySelector("#vstep3 .vbf-step-circle").textContent = "2";
                document.querySelector("#vstep3 .vbf-step-label").textContent =
                    translatedText("visitManualStepAppointment");
            ...
            function enableManualLocationMode() {
                manualLocation = true;
                document
                    .querySelectorAll(".manual-location-only")
                    .forEach(function (element) {
                        element.classList.remove("d-none");
                    });
                document
                    .querySelectorAll(".map-address-only")
                    .forEach(function (element) {
            ...
            document
                .getElementById("toStep3Btn")
                .addEventListener("click", function () {
                    goToStep(3);
                });
            document
                .getElementById("backToStep2Btn")
                .addEventListener("click", function () {
                    goToStep(2);
                });
            ...
            function populateAddressFields() {
                ...
                if (!streetEl._userEdited)
                    streetEl.value = document.getElementById("street_field").value;
                if (!cityEl._userEdited)
                    cityEl.value = document.getElementById("city_field").value;
                if (!zipEl._userEdited)
                    zipEl.value = document.getElementById("zip_field").value;

                if (selectedDistrictId) {
                    var d = allDistricts.find(function (x) {
                        return x.id === selectedDistrictId;
                    });
            ...
            function checkUnitInput() {
                ...
                var region = regionSelect.classList.contains("d-none")
                    ...
                    : regionSelect.value;
                var city = manualLocation
                    ? document.getElementById("manual_city_input").value.trim()
                    : true;
                var district = manualLocation
            ...
            document
                .getElementById("street_input")
                .addEventListener("input", checkUnitInput);
            document
                .getElementById("unit_input")
                .addEventListener("input", checkUnitInput);
            document
                .getElementById("manual_city_input")
                .addEventListener("input", checkUnitInput);
            document
                .getElementById("district_select")
                .addEventListener("change", checkUnitInput);
            ...
            function updateRegionOptions() {
                ...
                Array.from(regionSelect.options).forEach(function (option, index) {
                    if (!index) return;
                    var matches =
                        Boolean(stateId) && option.dataset.stateId === stateId;
                ...
                Array.from(districtSelect.options).forEach(function (option, index) {
                    ...
                    var matches =
                        Boolean(stateId) &&
                        option.dataset.regionId === regionSelect.value;
                    option.hidden = !matches;
                    option.disabled = !matches;
                });
            ...
            document
                .getElementById("state_select")
                .addEventListener("change", updateRegionOptions);
            document
                .getElementById("region_select")
                .addEventListener("change", function () {
                    var regionId = this.value;
                    Array.from(
                        document.getElementById("district_select").options
                    ).forEach(function (option, index) {
                        if (!index) return;
                        var matches =
                            Boolean(regionId) && option.dataset.regionId === regionId;
                        option.hidden = !matches;
                        option.disabled = !matches;
                    });
                    document.getElementById("district_select").value = "";
            ...
            function checkDistrictCoverage(lat, lng, address) {
                ...
                if (!matched && googleDistrictName) {
                    matched =
                        nameMatch(googleDistrictName, allDistricts) ||
                        nameMatch(
                            translatedText("visitDistrictPrefix") +
                                " " +
                                googleDistrictName,
                            allDistricts
                        );
            ...
            function reverseGeocode(lat, lng) {
                ...
                geocoder.geocode(
                    {location: {lat: lat, lng: lng}, language: "ar"},
            ...
            function setLocateButton(button, loading) {
                ...
                icon.className = loading
                    ...
                    : "fa fa-crosshairs me-1";
                button.replaceChildren(
                    icon,
                    document.createTextNode(
                        loading
                            ? translatedText("visitLocating")
                            : translatedText("visitMyLocation")
                    )
                );
            }
            ...
            async function loadRoutes() {
                ...
                try {
                    ...
                    empty.querySelector("p").textContent =
                        translatedText("visitRouteLoadError");
                ...
                days.forEach(function (d) {
                    ...
                    if (slots.length) {
                        slots.forEach(function (slot) {
                            ...
                            var routeName =
                                slot.route_name || translatedText("visitAppointment");
                            tag.type = "button";
                            tag.className = "route-tag";
                            tag.dataset.routeId = String(slot.id);
                            tag.dataset.routeName =
                                dayNames[d.getDay()] +
                                " " +
                                d.getDate() +
                                " " +
                            ...
                            tag.addEventListener("click", function () {
                                document
                                    .querySelectorAll("#vbf .route-tag")
                                ...
                                document.getElementById(
                                    "selectedSlotInfo"
                                ).textContent = tag.dataset.routeName;
                                document
                                    .getElementById("selectedSlotDisplay")
                                    .classList.remove("d-none");
                                document
                                    .getElementById("submitVisitBtn")
            ...
            function setSubmitButton(button, loading) {
                ...
                button.replaceChildren(
                    ...
                    document.createTextNode(
                        loading
                            ? translatedText("visitBooking")
                            : translatedText("visitConfirm")
                    )
                );
            }
            ...
            function showSubmitError(message) {
                ...
                document.getElementById("visitInlineErrorMessage").textContent =
                    message;
                ...
                .addEventListener("click", async function () {
                    ...
                    try {
                        var data = await rpc("/my/visit/submit", {
                            ...
                            city: manualLocation
                                ? document.getElementById("manual_city_input").value
                        ...
                        if (data.success) {
                            ...
                            showSubmitError(
                                data.error || translatedText("visitSubmitError")
                            );
                            btn.removeAttribute("disabled");
